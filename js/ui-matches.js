...
function renderMatchCard(fixture, allMatches, dayNumber) {
    ...
    const homeScore = isPlayed ? playedMatch.homeScore : '';
    const awayScore = isPlayed ? playedMatch.awayScore : '';
    ...
    return `
        <div class="${cardClasses}" data-home="${fixture.home}" data-away="${fixture.away}">
            ...
            <form class="match-score-form" data-home="${fixture.home}" data-away="${fixture.away}" data-doc-id="${docId}">
                <label for="${homeTeamLogo}-${awayTeamLogo}-score-home" class="visually-hidden">Home Score</label>
                <input type="number" id="${homeTeamLogo}-${awayTeamLogo}-score-home" class="score-home" min="0" value="${homeScore}" required>
                <span>-</span>
                <label for="${homeTeamLogo}-${awayTeamLogo}-score-away" class="visually-hidden">Away Score</label>
                <input type="number" id="${homeTeamLogo}-${awayTeamLogo}-score-away" class="score-away" min="0" value="${awayScore}" required>
            </form>
    ...
    const adminActionsHTML = appState.isAdmin ? `
        <div class="match-actions">
            <button type="button" class="btn btn-edit" aria-label="Add or Edit score for ${fixture.home} versus ${fixture.away}">
                ${isPlayed ? 'Edit' : 'Add Score'}
            </button>
            
            ${isPlayed ? `<button type="button" class="btn btn-delete" data-doc-id="${docId}" aria-label="Delete score for ${fixture.home} versus ${fixture.away}">Delete</button>` : ''}
            
            <button type="button" class="btn btn-save" aria-label="Save score change for ${fixture.home} versus ${fixture.away}">
                <span class="btn-text">Save</span>
            </button>
            <button type="button" class="btn btn-cancel" aria-label="Cancel editing score for ${fixture.home} versus ${fixture.away}">
                <span class="btn-text">Cancel</span>
            </button>
        </div>
    ` : '';
