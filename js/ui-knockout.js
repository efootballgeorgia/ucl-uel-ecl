...
/**
 * Checks if all teams in the current league have played the required number of matches.
 * @returns {boolean} True if all teams are ready, otherwise false.
 */
function areAllTeamsReadyForKnockout() {
    ...
    // Check every team defined in the config
    for (const teamName of leagueConfig.teams) {
        const row = dom.leagueTableBody.querySelector(`tr[data-team="${teamName}"]`);
        
        // If a team row doesn't exist or is a skeleton, they are not ready
        if (!row || row.classList.contains('skeleton')) {
            return false;
        }

        // The 'Played' count is in the 3rd cell (index 2)
        const playedGames = parseInt(row.cells[2].textContent, 10);
        
        // If the parsed number is not valid or is less than required, they are not ready
        if (isNaN(playedGames) || playedGames < requiredGames) {
            return false;
        }
    }
    
    // If the loop completes, all teams have met the condition
    return true;
}
