...
function renderMatchCard(fixture, allMatches) {
    ...
    if (isPlayed) {
        // REFINEMENT: Simplified score assignment
...
export function filterMatches(allMatches = appState.currentLeagueMatches) {
    ...
    const teamFixtures = allFixtures.flat().filter(fixture => 
    ...
    const html = teamFixtures.map(fixture => renderMatchCard(fixture, allMatches)).join('');
    dom.matchDayContainer.innerHTML = html;
    dom.noSearchResults.style.display = 'none';
}
...
// ============================================
// 4. Knockout Stage (REFACTORED)
// ============================================

/**
 * Calculates the entire knockout stage data structure without rendering HTML.
 * @param {string[]} sortedTeams - Array of team names in league order.
 * @param {object[]} knockoutMatches - Array of match objects from Firestore.
 * @returns {object[]|null} A structured array of rounds and matches, or null if not possible.
 */
...
/**
 * Renders the Knockout Stage UI based on calculated data.
 * @param {string[]} sortedTeams - Array of team names in league order.
 * @param {object[]} knockoutMatches - Array of match objects from Firestore.
 */
