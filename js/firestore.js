import { collection, doc, getDoc, deleteDoc, addDoc, setDoc, query, orderBy, onSnapshot, serverTimestamp } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js";
...
export async function handleMatchSubmission(form, button) {
    ...
    try {
        if (docId) {
            ...
            const existingMatch = appState.currentLeagueMatches.find(m => 
                (m.homeTeam === matchData.homeTeam && m.awayTeam === matchData.awayTeam)
            );
            if (existingMatch) {
                await setDoc(doc(appState.db, collectionName, existingMatch.id), matchData, { merge: true });
                showFeedback('Match updated successfully!', true);
            } else {
